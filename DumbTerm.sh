...
func runXCommand() {
   ...
   echo "( $1 || ( xterm -maximized -e \"sleep 0.1; NEWT_COLORS='root=,red' whiptail --msgbox -title 'Error' -geometry +0+0 'Failed to connect' $h $w --title Error\" &&  killall `echo $WM_COMMAND | grep -o -P '.*\W'` ) ) $S" >> ~/.xinitrc # Add the command to xinitrc
